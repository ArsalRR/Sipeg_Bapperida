...
<div id="modalUpload" class="fixed inset-0 z-[9999] hidden bg-slate-900/60 backdrop-blur-sm overflow-y-auto p-4 sm:p-6 md:p-10 flex items-center justify-center min-h-screen">
    <div class="bg-white dark:bg-[#18181b] rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100 dark:border-gray-800 transform transition-all my-auto">
        ...
        <form action="{{ route('dokumen.file.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4" onsubmit="return handleFormUploadSubmit(this, event)">
            @csrf
            ...
            <div class="pt-3">
                <button type="submit" class="w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-bold text-sm rounded-xl shadow-lg shadow-blue-500/30 transition-all uppercase tracking-wider flex items-center justify-center gap-2">
...
<script>
    ...
    function handleFormUploadSubmit(form, event) {
